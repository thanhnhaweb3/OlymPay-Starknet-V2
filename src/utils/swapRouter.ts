...
import { CONTRACT_ADDRESSES, DEFAULT_EXCHANGE_RATE, DEFAULT_SLIPPAGE } from '@/config/contracts'
...
export class SwapRouter {
  ...
  constructor(account: AccountInterface, provider: ProviderInterface) {
    ...
    this.vaultContract = new Contract(VAULT_ABI, CONTRACT_ADDRESSES.VAULT, provider)
  }

  /**
   * Get token balance for a user
   */
  async getTokenBalance(tokenAddress: string, userAddress: string): Promise<any> {
    try {
      const tokenContract = new Contract(ERC20_ABI, tokenAddress, this.provider)
      const balance = await tokenContract.balanceOf(userAddress)
      if (typeof window !== 'undefined') {
        console.log('Raw balance from contract:', balance, 'Type:', typeof balance)
      }
      return balance
    } catch (error) {
   ...
   */
  async checkAllowance(tokenAddress: string, userAddress: string): Promise<string> {
    try {
      const tokenContract = new Contract(ERC20_ABI, tokenAddress, this.provider)
   ...
   */
  async approveToken(tokenAddress: string, amount: string): Promise<SwapResult> {
    try {
      const tokenContract = new Contract(ERC20_ABI, tokenAddress, this.account)
      const result = await tokenContract.approve(CONTRACT_ADDRESSES.VAULT, amount)
  ...
  async executeSwap(params: SwapParams): Promise<SwapResult> {
    try {
      ...
      const minAmountOut = (Number(amountOut) * (100 - slippageTolerance) / 100).toString()
      
      // Convert amounts to proper format
      const amountInFormatted = (Number(amountIn) * Math.pow(10, tokenInInfo.decimals)).toString()
      const minAmountOutFormatted = (Number(minAmountOut) * Math.pow(10, tokenOutInfo.decimals)).toString()
   ...
   */
  async depositToken(tokenAddress: string, amount: string): Promise<SwapResult> {
    try {
      const tokenInfo = this.getTokenInfo(tokenAddress)
      const amountFormatted = (Number(amount) * Math.pow(10, tokenInfo.decimals)).toString()
      
      const result = await this.vaultContract.deposit(tokenAddress, amountFormatted)
   ...
   */
  async withdrawToken(tokenAddress: string, amount: string): Promise<SwapResult> {
    try {
      const tokenInfo = this.getTokenInfo(tokenAddress)
      const amountFormatted = (Number(amount) * Math.pow(10, tokenInfo.decimals)).toString()
      
      const result = await this.vaultContract.withdraw(tokenAddress, amountFormatted)
